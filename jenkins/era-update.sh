...
python grits_flight_pull.py -m 172.30.2.95 -d grits-net-meteor
...
python grits_consume.py --type DiioAirport -m 172.30.2.95 -d grits-net-meteor tests/data/MiExpressAllAirportCodes.tsv
python grits_consume.py --type FlightGlobal -m 172.30.2.95 -d grits-net-meteor data/EcoHealth_*.csv  &&\
rm -fr data

echo "*****Make sure indexes are good*****"
python grits_ensure_index.py -d grits-net-meteor -m 172.30.2.95 -f

echo "*****Import heat map data*****"
aws s3 sync s3://flight-network-heat-map/ ./
mongorestore -h 172.30.2.95 -d grits-net-meteor -c heatmap ./dump/grits/heatmap.bson

echo "*****Create legs collection*****"
java -jar /flirt-legs.jar --mongohost="172.30.2.95" mongoport="27017"
